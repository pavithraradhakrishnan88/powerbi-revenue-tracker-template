...
if($Gate -eq 'report' -or $Gate -eq 'assembly'){
 ...
 foreach($p in @((Join-Path $report '.platform'),(Join-Path $report 'definition.pbir'),(Join-Path $report 'definition/pages/pages.json'))){if(!(Test-Path $p -PathType Leaf)){throw "STRUCTURE-REPORT-GATE failed: missing '$p'."}}
 ...
 Write-Host "STRUCTURE-REPORT-GATE|PASS|pages=$(@($pages.pageOrder).Count)|json=valid|platform=present";if($Gate -eq 'report'){exit 0}
}
